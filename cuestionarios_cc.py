...
cols_permitidos = [
    ...
    *[f"CEM{str(i).zfill(2)}" for i in range(1, 17)],
    *[f"CV{str(i).zfill(2)}" for i in range(1, 49)],
...
for idx, row in df.iterrows():
    ...
    print(f"\n---- Alumno {alumno_id} ---- código clase: {codigo_publico}")
    ...
    for col in cols_permitidos:
        ...
        if v == 9:
            continue  # no enviar "no responde"

        payload_cc[col] = v

    print("Payload CC:", payload_cc)
    ...
    try:
        ...
        cc_error += 1
        print("ERROR EXCEPTION enviando CC:", e)
