...
# --- Config ---
engine = create_engine('mysql+pymysql://root:password@localhost/soled3m')  # Cambia a 'soled3m' del DDL
...
# --- Leer Excel ---
df = pd.read_excel('data.xlsx', sheet_name='data', header=1)  # Header=1 para usar row2 como columnas

with engine.begin() as conn:
    # 1. CENTROS
    centros_raw = df[['centro_nombre', 'centro_tipo', 'centro_calle', 'centro_numero',
                      'centro_poblacion', 'centro_provincia', 'centro_pais', 'centro_cod_postal']].drop_duplicates()
    ...
    centro_map = dict(zip(centros_raw['centro_nombre'], centros['codigo_publico_centro']))
    ...
    prof_raw = df[['prof_nombre', 'prof_apellidos', 'prof_edad', 'prof_sexo',
                   ...
                   'prof_contrasenya', 'centro_nombre', 'curso_escolar_id']].drop_duplicates()
    ...
    profesores.columns = ['nombre', 'apellidos', 'edad', 'sexo', 'nacionalidad',
                          ...
                          'asignatura', 'email', 'contrasenya', 'centro_nombre', 'curso_escolar_id']
    profesores['codigo_publico_centro'] = profesores['centro_nombre'].map(centro_map)
    profesores['acuerdo_confidencialidad'] = True
    profesores['activo'] = 1
    profesores = profesores.drop(columns=['centro_nombre'])
    ...
    clase_map = dict(zip(zip(clases_raw['clase_curso'], clases_raw['prof_email']), clases['codigo_publico_clase']))
    ...
    key = list(zip(alumnos['clase_curso'], alumnos['prof_email']))
    ...
    # 6. CUESTIONARIO CE
    ce_items = [col for col in df.columns if col.startswith(('D', 'P', 'A', 'F', 'SEG', 'CFIS', 'CR')) and len(col) >= 5]
    ce_data = df[['prof_email'] + ce_items].dropna(subset=['prof_email'])
    ce_data['id_profesor'] = ce_data['prof_email'].map(prof_map)
    ce_data['id_centro'] = df['centro_nombre'].map(centro_map)
    ...
    # 7. CUESTIONARIO CC
    cc_items = [col for col in df.columns if (col.startswith('auton') or col.startswith('condeval') or col.startswith('CDC') or col.startswith('CEM') or col.startswith('CV'))]
    cc_data = df[['clase_curso', 'prof_email'] + cc_items].copy()
    key = list(zip(cc_data['clase_curso'], cc_data['prof_email']))
...
print("Datos volcados exitosamente.")